...
if ($pg == 'login') {
    ...
    $siswaQ = mysqli_query($koneksi, "SELECT * FROM daftar WHERE (nisn = '$username' OR nama = '$username')");
...
if ($pg == 'login2') {
    ...
    $username = mysqli_escape_string($koneksi, $_POST['username']);
    $password = mysqli_escape_string($koneksi, $_POST['password']);
    $siswaQ = mysqli_query($koneksi, "SELECT * FROM siswa WHERE (nisn = '$username' OR nama = '$username')");
    if ($username <> "" and $password <> "") {
        if (mysqli_num_rows($siswaQ) == 0) {
            $data = [
                'pesan' => 'Anda belum terdaftar silahkan Hubungi Operator Sekolah!'
